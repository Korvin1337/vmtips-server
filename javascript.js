...
const loadFromCloud = async () => {
    ...
    if (!data.record) throw new Error("Tomt svar från JSONBin – inget 'record' fält.");
    return data.record;
};
...
const App = () => {
    ...
    const handleManualCloudLoad = () => {
        setModalConfig({
            ...
            message: "Detta ersätter all lokal data med det som finns sparat i molnet. Vill du fortsätta?",
            type: "confirm",
            onConfirm: () => {
                setModalConfig(null);
                setSyncStatus(SYNC.LOADING);
    ...
    const handleAddPlayer = (e) => {
        ...
        requireAdmin(() => {
            ...
            if (!val) return;
            setState(s => s.players[val] ? s : {
                ...s, players: { ...s.players, [val]: { tips: {}, locked: false } }
            });
    ...
    const updateTip = (id, field, val) => {
        ...
        if (isNaN(v)) v = ""; else v = Math.max(0, Math.min(9, v));
        setState(s => {
    ...
    const adjustScore = (id, field, currentVal, delta, isActual, date) => {
        ...
        const cur = (currentVal !== undefined && currentVal !== "") ? parseInt(currentVal, 10) : 0;
        ...
        requireAdmin(() => {
            setModalConfig({
                ...
                onConfirm: () => {
                    ...
                    setState(s => {
                        updatedStateToSave = { ...s, lockedDays: { ...s.lockedDays, [date]: !s.lockedDays?.[date] } };
                        ...
                        if (updatedStateToSave) {
                            ...
                            saveToCloud(updatedStateToSave)
                                .then(() => {
                                    setLastSynced(new Date());
                                    setSyncStatus(SYNC.SAVED);
                                    localStorage.setItem("vmtips_v3", JSON.stringify(updatedStateToSave));
                                })
                                .catch(err => {
                                    setSyncError(err.message);
                                    setSyncStatus(SYNC.ERROR);
            ...
            reader.onload = (ev) => {
                try {
                    ...
                    setModalConfig({
                        ...
                        message: "Vill du ersätta all nuvarande data med innehållet i filen?",
                        type: "confirm",
                        onConfirm: () => { setState(imported); setModalConfig(null); }
                    });
                } catch (_) {
                    setModalConfig({ title: "Fel", message: "Ogiltig JSON-fil.", type: "error" });
                }
            };
    ...
    const leaderboard = useMemo(() => {
        return Object.entries(state.players).map(([name, data]) => {
            ...
            fixturesData.forEach(f => {
                ...
                pts += calculateMatchScore(act.h, act.a, tip.h, tip.a, state.lockedDays?.[f.date] || false);
            });
    ...
    const SyncPill = () => {
        const t = lastSynced
            ...
            : null;
        const configs = {
            [SYNC.LOADING]: { dot: "bg-blue-400 animate-pulse",  text: "text-blue-400",   label: "Laddar från moln…" },
            [SYNC.SAVING]:  { dot: "bg-yellow-400 animate-pulse",text: "text-yellow-400", label: "Sparar till moln…" },
            [SYNC.SAVED]:   { dot: "bg-emerald-400",             text: "text-emerald-400",label: `Molnsynk OK${t ? ' · ' + t : ''}` },
            [SYNC.ERROR]:   { dot: "bg-rose-400",                text: "text-rose-400",   label: "Molnfel – lokalt sparat" },
            [SYNC.IDLE]:    { dot: "bg-slate-600",               text: "text-slate-500",  label: "Inte synkad" },
        };
        const c = configs[syncStatus];
        return (
            <span className={`flex items-center gap-1.5 text-xs font-medium ${c.text}`} title={syncError || undefined}>
                <span className={`inline-block w-2 h-2 rounded-full shrink-0 ${c.dot}`}></span>
                {c.label}
            </span>
        );
    };
        ...
        return (
            <div className="flex items-center bg-slate-900 border border-slate-700 rounded-lg p-1 select-none">
                <button type="button"
                    ...
                    className="w-7 h-7 flex items-center justify-center bg-slate-800 hover:bg-slate-700 active:bg-slate-600 text-slate-300 disabled:text-slate-600 rounded font-black text-sm transition-colors disabled:opacity-40">+</button>
                ...
                <button type="button"
                    ...
                    onClick={() => adjustScore(id, field, v, -1, isActual, date)}
                    className="w-7 h-7 flex items-center justify-center bg-slate-800 hover:bg-slate-700 active:bg-slate-600 text-slate-300 disabled:text-slate-600 rounded font-black text-sm transition-colors disabled:opacity-40">&minus;</button>
            </div>
        );
    };

    const renderPlayerControlPanel = () => {
    ...
    if (!hasAttemptedLogin) return (
        <div className="flex flex-col items-center justify-center min-h-screen p-4 relative gap-12">
            ...
            <div className="w-full max-w-sm opacity-30 hover:opacity-100 transition-opacity duration-300">
                <form onSubmit={handleLoginSubmit} className="flex flex-col gap-3">
                    ...
                    <input type="password" value={loginAttempt} onChange={e => setLoginAttempt(e.target.value)}
            ...
            <div className="absolute bottom-4 right-4 opacity-0 pointer-events-none select-text text-[10px]">VMTIPS2026</div>
        </div>
    );
            ...
            <table className="w-full text-left border-collapse text-xs">
                <thead>
                    <tr className="bg-slate-900/80">
                        ...
                        {playerKeys.map(p => <th key={p} className="p-2 border-b border-slate-700 text-slate-200 font-bold text-center border-l border-slate-700/50">{p}</th>)}
                    </tr>
                </thead>
                <tbody>
                    {fixturesData.map(f => {
                                        ...
                                        <td key={p} className="p-2 text-center border-l border-slate-700/50">
                                            {tipStr
                                                ? <div className="flex flex-col items-center">
                                                    ...
                                                    {locked && <span className={`text-[10px] ${pts > 0 ? 'text-emerald-400 font-bold' : 'text-slate-500'}`}>{pts} pt</span>}
                                                  </div>
    ...
    if (printMode !== null && printMode !== 'overview') {
        ...
        return (
            <div className="p-4 bg-slate-900 text-slate-100 min-h-screen">
                ...
                {!leaderboardOnly && toPrint.map((pK, i) => (
                    <div key={pK} className={`mb-12 ${i < toPrint.length - 1 ? 'print-page-break' : ''}`}>
                                        ...
                                        return (
                                            <div key={f.id} className="bg-slate-800 border border-slate-700 rounded-lg p-3 flex flex-row items-center gap-4">
                                                <div className="w-20 shrink-0">
                                                    ...
                                                    <div className="text-xs text-emerald-400">{f.tv}</div>
                                                </div>
                                                <div className="flex-1 flex items-center justify-between gap-4">
                                                    <div className="text-right flex-1 font-medium text-slate-200">{f.home}</div>
                                                    {has
                                                        ? <div className="flex items-center gap-2 bg-slate-950 px-3 py-1.5 rounded border border-slate-700">
                                                            <span className="w-6 text-center text-lg font-bold text-white">{tH}</span>
                                                            <span className="text-slate-500 font-extrabold">&ndash;</span>
                                                            <span className="w-6 text-center text-lg font-bold text-white">{tA}</span>
                                                          </div>
                                                        : <div className="flex items-center justify-center bg-slate-900 px-3 py-1.5 rounded border border-slate-700/50 w-24">
                                                            <span className="text-slate-600 font-bold">–</span>
                                                          </div>}
                                                    <div className="text-left flex-1 font-medium text-slate-200">{f.away}</div>
                                                </div>
                ...
                <div className="mt-12 bg-slate-800 rounded-xl p-6 border border-slate-700 print-avoid-break">
                    <h2 className="text-2xl font-bold text-emerald-400 mb-4 uppercase text-center">Aktuell Leaderboard</h2>
                    <table className="w-full text-left border-collapse">
                        <thead><tr className="bg-slate-900/50">
                            <th className="p-3 border-b border-slate-700 text-slate-400">Placering</th>
                            <th className="p-4 border-b border-slate-700 text-slate-400">Spelare</th>
                            <th className="p-3 border-b border-slate-700 text-right text-emerald-400">Poäng</th>
                        </tr></thead>
                        <tbody>
                            {leaderboard.map((l, i) => (
                                <tr key={l.name} className="border-b border-slate-700/50">
                                    <td className="p-3 text-lg font-bold text-slate-300">#{i + 1}</td>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
                    <div className="bg-slate-800 border border-slate-700 rounded-xl shadow-2xl p-6 w-full max-w-md relative">
                        ...
                        <div className="flex justify-end gap-3">
                            {modalConfig.type !== 'custom' && modalConfig.type !== 'edit-name' && (
                                <button onClick={() => setModalConfig(null)}
                            ...
                            {(modalConfig.type === 'confirm' || modalConfig.type === 'custom' || modalConfig.type === 'edit-name') && (
                                <button onClick={modalConfig.type === 'edit-name' ? () => confirmNameChange(browsedPlayer.trim()) : modalConfig.onConfirm}
            ...
            <header className="bg-slate-800 rounded-xl p-4 md:p-6 mb-6 shadow-lg border border-slate-700 flex flex-col md:flex-row justify-between items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-400 text-sm">
                        Läge: <span className={`font-bold ${isAuthenticated ? 'text-emerald-400' : 'text-slate-300'}`}>{isAuthenticated ? 'Admin' : 'Åskådare'}</span>
                    </p>
                ...
                <div className="flex flex-col gap-5">
                    ...
                    <div className="bg-slate-800 rounded-xl border border-slate-700 p-6 flex flex-col gap-5">
                        <div className="flex items-start justify-between gap-4 flex-wrap">
                            <div>
                                <h2 className="text-lg font-bold text-emerald-400">JSONBin Molnsynk</h2>
                                <p className="text-slate-400 text-sm mt-1">
                                    Data laddas automatiskt från molnet när du öppnar sidan.<br/>
                                    <strong className="text-slate-300">Spara till molnet</strong>
                                </p>
                            </div>
                            <SyncPill />
                        </div>
                        ...
                        {syncStatus === SYNC.ERROR && (
                            <div className="bg-rose-500/10 border border-rose-500/30 rounded-lg p-4">
                                <p className="text-rose-400 text-sm font-bold mb-1">Fel vid molnkommunikation</p>
                                <p className="text-rose-300/80 text-xs break-all">{syncError}</p>
                            </div>
                        )}
                        ...
                        <div className="flex gap-3 flex-wrap">
                            <button
                                ...
                                disabled={syncStatus === SYNC.SAVING || syncStatus === SYNC.LOADING}
                                ...
                                {syncStatus === SYNC.SAVING ? "Sparar…" : "⬆  Spara till JSONBin"}
                            </button>
                            <button
                                ...
                                {syncStatus === SYNC.LOADING ? "Laddar…" : "⬇  Ladda från JSONBin"}
                            </button>
                        </div>
                    </div>
                ...
                <div className="flex flex-col gap-6">
                            ...
                            <tbody>
                                {leaderboard.map((l, i) => (
                                    <tr key={l.name} className="hover:bg-slate-700/50 transition-colors">
                                        <td className="p-4 border-b border-slate-700 text-xl font-bold text-slate-300">#{i + 1}</td>
                                        <td className="p-4 border-b border-slate-700 font-bold">{l.name}</td>
                                        <td className="p-4 border-b border-slate-700">
                                            {l.locked
                                                ? <span className="text-xs bg-rose-500/20 text-rose-400 px-2 py-1 rounded border border-rose-500/30">Låst</span>
                                                : <span className="text-xs bg-emerald-500/20 text-emerald-400 px-2 py-1 rounded border border-emerald-500/30">Öppen</span>}
                                        </td>
                                        <td className="p-4 border-b border-slate-700 text-right font-bold text-xl text-emerald-400">{l.pts}</td>
                                    </tr>
                                ))}
                                {leaderboard.length === 0 && (
                                    <tr><td colSpan="4" className="p-8 text-center text-slate-500">Inga spelare har lagts till än.</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
                    {playerKeys.length > 0 && (
                        <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 flex justify-center">
                            <button onClick={() => setPrintMode('leaderboard')}
                ...
                <div className="flex flex-col gap-6">
                    <div className="bg-slate-800 rounded-xl shadow-lg border border-slate-700 overflow-x-auto">
                        <table className="w-full text-left border-collapse whitespace-nowrap text-sm">
                            <thead><tr className="bg-slate-900/80">
                                ...
                                {playerKeys.map(p => <th key={p} className="p-3 border-b border-slate-700 text-slate-200 font-bold text-center border-l border-slate-700/50">{p}</th>)}
                            </tr></thead>
                            <tbody>
                                {fixturesData.map(f => {
                                    const act = state.actual[f.id] || {};
                        ...
                        <div className="bg-slate-800 p-4 rounded-xl border border-slate-700 shadow-md flex flex-col gap-4 mb-6">
                            <div>
                                ...
                                <form onSubmit={handleAddPlayer} className="flex flex-col md:flex-row gap-3">
                                    ...
                                    <button type="submit" disabled={!newPlayerName.trim() || !isAuthenticated}
                            ...
                            {playerKeys.length > 0 && (
                                <div>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {playerKeys.map(p => (
                                            <button key={p} type="button" onClick={() => handlePlayerChange(p)}
                        ...
                        return (
                            <div key={date} className="mb-8">
                                <div className="flex justify-between items-center mb-4 border-b border-slate-700 pb-2">
                                    ...
                                    {tab === "admin" && (
                                        <button onClick={() => toggleDayLock(date)}
                                ...
                                <div className="grid gap-3">
                                    {matches.map(f => {
                                        ...
                                        const key = browsedPlayer.trim();
                                        const matchData = isTips ? (state.players[key]?.tips[f.id] || {}) : (state.actual[f.id] || {});
                                        const act = state.actual[f.id] || {};
                                        const isInputDisabled = isTips
                                            ? (!key || state.players[key]?.locked || !isAuthenticated)
                                            : (isDayLocked || !isAuthenticated);
                                        const pts = isTips ? calculateMatchScore(act.h, act.a, matchData.h, matchData.a, state.lockedDays?.[f.date]) : null;
                                        return (
                                            <div key={f.id} className={`bg-slate-800 border ${tab === 'admin' && isDayLocked ? 'border-rose-500/50' : 'border-slate-700'} rounded-lg p-4 flex flex-col md:flex-row items-center gap-4 shadow-sm hover:border-slate-600 transition-colors`}>
                                                ...
                                                <div className="flex-1 flex items-center justify-between gap-4 w-full">
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        <ScoreInputGroup id={f.id} field="h" val={matchData.h} disabled={isInputDisabled} isActual={!isTips} date={f.date} />
                                                        <span className="text-slate-500 font-extrabold text-lg">&ndash;</span>
                                                        <ScoreInputGroup id={f.id} field="a" val={matchData.a} disabled={isInputDisabled} isActual={!isTips} date={f.date} />
                                                    </div>
                        ...
                        <div className="mt-8 mb-4 bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-md flex flex-col gap-6 items-center text-center">
                            ...
                            <div className="w-full">
                                ...
                                <div className="flex flex-col sm:flex-row justify-center items-center gap-3 flex-wrap">
                                    <select value={selectedPrintDate} onChange={e => setSelectedPrintDate(e.target.value)}
                                    ...
                                    {browsedPlayer.trim() && state.players[browsedPlayer.trim()] && (
                                        <button onClick={() => setPrintMode('day-single')}
                                    ...
                                    {playerKeys.length > 0 && (
                                        <button onClick={() => setPrintMode('day-all')}
